...
if __name__ == "__main__":
	...
	datasets = [directory for directory in os.listdir(path) if os.path.exists(path + directory + "/distances_sample")]
	configs = set()
	...
	for i, dataset in enumerate(datasets):
		for j, config in enumerate(configs):
			if os.path.exists(path + dataset + "/distances_sample/" + dataset + "_" + config + ".distances"):
				...
				score = train_svm(path + dataset + "/distances_sample/" + dataset + "_" + config + ".distances")
	...
	with open(path + "accuracies_sample_new.csv", "w") as file:
